...
void awake_tasks (task_t *queueJOIN, int tamanho);
//--------------------------------------------------------------------------------------------------------------//
...
void ppos_init () //INICIALIZA AS ESTRUTURAS INTERNAS DO SISTEMA OPERACIONAL
{
  ...
  timer.it_value.tv_usec = 100;        // primeiro disparo, em micro-segundos (90000 microssegundos = 0,0001 segundo)
      ...
      switch (taskNext->taskState)
      {
        ...
        case 4: //CASE: TERMINATED
        {
          if (taskNext->contJOIN)
            awake_tasks (taskNext->queueJOIN, taskNext->contJOIN);
