...
{
  if ((queue->buffer = malloc (max_msgs * msg_size)))
  {
    queue->queue_size = max_msgs;
    queue->message_bsize = msg_size;
    queue->start = 0;
    queue->end = 0;
    queue->msgState = AVAILABLE;
    sem_create (&queue->s_vaga, 1);
    sem_create (&queue->s_buffer, max_msgs);
...
{
  if (!queue || queue->msgState == UNAVAILABLE)
    return -1;
  
  sem_down (&queue->s_vaga);
  if (queue->msgState == UNAVAILABLE)
    return -1;
  sem_down (&queue->s_buffer);
  
  memcpy (queue->buffer + (queue->end * queue->message_bsize), msg, queue->message_bsize);
  queue->end = (queue->end + 1) % queue->queue_size;
  queue->message_counter ++;
...
int mqueue_recv (mqueue_t *queue, void *msg)
{
  ...
  memcpy (msg, queue->buffer + (queue->start * queue->message_bsize), queue->message_bsize);
